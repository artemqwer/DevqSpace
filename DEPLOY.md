...
## Оплата карткою — банка Monobank (без ФОП)

Найпростіший варіант картки, без ФОП і еквайрингу. Мінус: банка не має API/вебхука, тому **оплату підтверджуєш вручну** (1 тап).

### Налаштування
1. У додатку **Monobank** → створи **банку** (jar) → «Поділитись» → скопіюй посилання (типу `https://send.monobank.ua/jar/XXXX`)
2. У Vercel додай змінні:
   | Назва | Значення |
   |---|---|
   | `MONOBANK_JAR_URL` | посилання на банку |
   | `USD_UAH_RATE` | курс $→грн для показу суми (напр. `42`) |
3. **Redeploy**

### Як працює
1. Клієнт на товарі тисне **«Сплатити карткою»** (ввівши ім'я+контакт) → бачить суму в грн, посилання на банку і номер замовлення
2. Переказує на банку, у коментарі лишає `#номер_замовлення`
3. Тобі в Telegram прилітає **«🟡 Очікує оплати на банку»** з кнопками
4. Бачиш надходження в Monobank → тиснеш **«💰 Оплачено»** (у Telegram або в `/admin/orders`) → замовлення закрите, видаєш товар

> Без `MONOBANK_JAR_URL` кнопка картки ховається. Можна тримати **обидва** способи одночасно — картку (банка) і крипту (NOWPayments).

## Telegram Support Bot (листування від імені сапорту)

У проекті передбачено **окремого Support-бота** спеціально для роботи з клієнтами (відокремлено від адмін-бота замовлень).

### Можливості:
* Сповіщення операторів про виклики клієнтів з віджету на сайті.
* **Відповідь клієнту напряму з Telegram:** оператор робить звичайний `Reply (Відповісти)` на сповіщення — і текст миттєво доставляється клієнту у веб-чат.
* Керування зверненням: кнопки `[🤖 Повернути AI]`, `[🔒 Закрити тікет]`, перегляд історії через `/ticket <id>` або `/tickets`.
* Робота в режимі омніканальності: якщо клієнт пише боту напряму в Telegram, оператор так само листується з ним через Reply.

### Налаштування на Vercel (1-2 хвилини):
1. Напишіть [@BotFather](https://t.me/BotFather) → `/newbot` → створіть другого бота (напр., `DevqSpace Support`). Скопіюйте його токен.
2. Згенеруйте секрети:
   ```bash
   openssl rand -hex 32  # для SUPPORT_TELEGRAM_WEBHOOK_SECRET
   openssl rand -hex 16  # для SUPPORT_TELEGRAM_SETUP_KEY
   ```
3. У Vercel додайте змінні середовища:
   | Назва | Значення |
   |---|---|
   | `SUPPORT_TELEGRAM_BOT_TOKEN` | Токен другого бота з @BotFather |
   | `SUPPORT_TELEGRAM_ADMIN_CHAT_ID` | ID чату/групи підтримки (дізнатися через команду `/where`) |
   | `SUPPORT_TELEGRAM_WEBHOOK_SECRET` | Згенерований 32-значний секрет |
   | `SUPPORT_TELEGRAM_SETUP_KEY` | Згенерований 16-значний ключ |
4. **Deploy / Redeploy**.
5. Активуйте webhook Support-бота у браузері:
   ```
   https://<your-domain>/api/support/tg-webhook/setup?key=<SUPPORT_TELEGRAM_SETUP_KEY>
   ```
6. Готово! Тепер при ескалації звернення в чаті сайту вам приходить повідомлення в Telegram, на яке можна відповідати звичайним Reply.

## Python-варіант (опційно, для self-hosted VPS)

У теці `bot/` лежать:
* `admin_bot.py` — бот замовлень та аналітики.
* `support_bot.py` — бот служби підтримки для роботи через long-polling.

Для деплою на Vercel вони не потрібні, адже обидва боти вже працюють через serverless webhooks.
